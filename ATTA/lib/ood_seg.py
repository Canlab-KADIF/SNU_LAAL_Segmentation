...
from lib.visualize import save_visualization
...
class OOD_Model(object):
    ...
    This function is fitted to ResNet101 backbone model.
    """
    ...
    def atta(self, img, ret_logit =False):
        ...
        anomaly_score, logit, dec0 = self.method.anomaly_score(img, ret_logit=True)
        ...
        with torch.no_grad():
            ...
            feature = self.method.model.module.final1(dec0)
            feature = self.method.model.module.final2(feature)
    ...
    def inference(self):
        ...
        for (i, data) in tqdm(enumerate(self.data_loaders['test'])):
            img, target = data[0].cuda(), data[1]
            if opt.train.tta == 'atta':
                anomaly_score, logit = self.atta(img, ret_logit=True)
            elif opt.train.tta == 'tent':
                anomaly_score, logit = self.tent(img, ret_logit=True)
            else:
                anomaly_score, logit = self.method.anomaly_score(img, ret_logit=True)
            anomaly_npy = anomaly_score.detach().cpu().numpy()
            anomaly_threshold = self.calculate_metrics(target.numpy(), anomaly_npy, i)
            ood_gts_list.append(target.numpy())
            anomaly_score_list.append(anomaly_npy)
    
            
            pred_labels = logit.argmax(dim=1)
            ...
            for idx in range(batch_size):
                ...
                save_visualization(
                img[idx],
                target[idx].cpu().numpy(),
                pred_labels[idx].cpu().numpy(),
                self.current_image_idx,
                dataset_name,
                method_name,
                anomaly_score=anomaly_score[idx].cpu().numpy(), 
                threshold=anomaly_threshold 
            )

            self.current_image_idx += 1
    
        roc_auc, prc_auc, fpr95, threshold = eval_ood_measure(np.array(anomaly_score_list), np.array(ood_gts_list))
    ...
    def get_domainshift_prob(self, x, threshold = 50.0, beta = 0.1, epsilon = 1e-8):
        ...
        for i, layer in enumerate(self.method.model.modules()):
            if isinstance(layer, nn.BatchNorm2d):
                mu_x, var_x = global_vars.get_global_mean_var()
              
                mu, var = global_vars.get_global_running_mean_var()
    ...
    def calculate_metrics(self, target, anomaly_npy, id):
        if (target == 1).sum() > 0:
            roc_auc, prc_auc, fpr95, threshold = eval_ood_measure(anomaly_npy, target, train_id_out=1)
