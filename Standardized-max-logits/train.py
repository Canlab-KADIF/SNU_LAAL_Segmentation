...
def apply_augmentation(input):
    return augmentation_transforms(input)

def ood_loss_function(predictions, ood_labels):
    # Example: Binary Cross-Entropy Loss for OOD detection
    return torch.nn.BCEWithLogitsLoss()(predictions, ood_labels)
...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    for i, data in enumerate(train_loader):
        ...
        inputs = apply_augmentation(inputs)
        ...
        for di, ingredients in enumerate(zip(inputs, seg_gts, ood_gts, aux_gts)):
            ...
            # Calculate OOD loss
            ood_loss = ood_loss_function(anomaly_score, ood_gt)
            
            total_loss = main_loss + (0.4 * aux_loss) + (0.1 * ood_loss)
